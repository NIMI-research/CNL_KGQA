...
if __name__ == '__main__':
    ...
    if args.language.lowercase() == "squall":
        ...
        processed_ground_truth = post_processing._post_process_squall(ground_truth)
        processed_manual_truth = post_processing._post_process_squall(predictions, gt = False, path =f'{args.path_to_save}squall_to_parse.txt')
        df_predictions = pd.DataFrame(processed_manual_truth)
        ...
        squall_to_sparql_tool = squall2sparql(input_final_path = f'{args.path_to_save}squall_to_parse.txt', output_final_path = f'{args.path_to_save}squall_parsed.txt')
        parsed_list = parse(f'{args.path_to_save}squall_parsed.txt', args.test_jsonl_file, args.path_to_prop)._parser_intermediate_sparql()
